confSAM <- function(p, PM, includes.id=TRUE, cutoff=0.01, reject="small", alpha=0.05,
  ...
  if(method=="full"){
    if(nrej[1]>50 | simple>8) { warning("the full closed testing procedure
                                        might be computationally infeasible.") }

    #make vector indR with indices corresponding to rejected set:
    Rset <- (p < cutoff)
    Rc <- (numeric(m)+1)-Rset    #complement of Rset
    indR <- numeric(1)
    for(i in 1:m){
      if(Rset[i]==TRUE){
        if(max(indR)==0){
    ...
    for( l in 1:nrej[1] ){
      if(boundfound==FALSE){
        ...
        while(boundfound==TRUE & i <= nrcombs){
          for(j in 1:w){
            nrejs[j] <- sum( PM[j, c(indRc, combs[,i])] < cutoff )
          }
          if(l <= sort(nrejs)[k]) {
    ...
    for( l in 1:nrej[1] ){
      if(boundfound==FALSE){
        ...
        while(boundfound==TRUE & i <= nrrandomcombs){
          for(j in 1:w){
            nrejs[j] <- sum( PM[j, c(indRc, rcombs[,i])] < cutoff )
          }
          if(l <= sort(nrejs)[k]) {
            boundfound <- FALSE   #the bound is not l
            appctbound <- l
  ...
  if(method=="csc"){  #conservative shortcut
    ...
    if(reject != "small"){ warning("The conservative shortcut is only useful if the test
                                   statistics are p-values and the smallest p-values
                                   are rejected") }

    ord <- order(nrej)

    Rset <- (p < cutoff)            #    Rset <- (P.unsorted < cutoff[l])
    S <- (PM[ord,]<cutoff)%*% Rset  #S <- (pvmatr.unsorted[ord,]<cutoff[l])%*% Rset
    U <- numeric(nrej[1])           #U <- numeric(nrejections[f,l])
